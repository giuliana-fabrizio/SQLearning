<template>
    <button @click="handleClickBtn" id="id_btn_logout" class="btn fw-bold ps-3 text-start">
        <svg class="position-absolute doorway" viewBox="0 0 100 100">
        ...
        <svg class="position-absolute figure" viewBox="0 0 100 100">
        ...
        <svg class="position-absolute door" viewBox="0 0 100 100">
            <path d="M93.4 86.3H58.6c-1.9 0-3.4-1.5-3.4-3.4V17.1c0-1.9 1.5-3.4 3.4-3.4h34.8c1.9 0 3.4 1.5 3.4 3.4v65.8c0 1.9-1.5 3.4-3.4 3.4z" />
            <circle cx="66" cy="50" r="3.7" />
        </svg>
        <span class="position-relative button-text">Se déconnecter</span>
    </button>
</template>
...
export default {
    ...
    data: () => ({
        ...
        button_state: {
            default: {
                ...
                "--transform-arm1": "none", "--transform-wrist1": "none", "--transform-arm2": "none",
                "--transform-wrist2": "none", "--transform-leg1": "none", "--transform-calf1": "none",
    ...
    mounted() {
        this.button = document.getElementById("id_btn_logout");
        this.button.state = "default";

        this.button.addEventListener("mouseenter", () => {
            if (this.button.state === "default") {
                this.updateButtonState(this.button, "hover");
            }
        });

        this.button.addEventListener("mouseleave", () => {
            if (this.button.state === "hover") {
                this.updateButtonState(this.button, "default");
            }
        });
    },
    ...
    methods: {
        async handleClickBtn() {
            if (this.button.state !== "default" && this.button.state !== "hover") return;

            this.button.classList.add("clicked");
            this.updateButtonState(this.button, "walking1");
            await this.wait(Number(this.button_state["walking1"]["--figure-duration"]));

            this.button.classList.add("door-slammed");
            this.updateButtonState(this.button, "walking2");
        ...
        updateButtonState(button, state) {
            if (this.button_state[state]) {
                button.state = state;
                for (let key in this.button_state[state]) {
                    button.style.setProperty(key, this.button_state[state][key]);
                }
            }
        },
