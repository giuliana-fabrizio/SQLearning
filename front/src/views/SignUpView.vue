<template>
    <div>
        <img alt="background" src="@/assets/background.png" class="img-fluid position-fixed w-100 img-auth">
        ...
        <div class="d-flex justify-content-center position-relative pt-4">
            <div class="col-10 col-sm-8 col-md-6 col-lg-4 mt-5 mb-5">
                ...
                <div>
                    <ProfileComponent @alert="displayAlert" @submit="submit" form_class="bg-form-auth"
                        input_class="bg-input-auth" />
                    ...
                    .then((credential) => {
                        axios.post(`http://localhost:${this.port}/user`, { user: { ...user_to_create, id: credential.user.uid } })
                            ...
                            .catch((error) => {
                                ...
                                this.updateAlert("alert-danger", error.message + " " + error.details, true);
                            });
                    })
                    .catch((error) => {
                        this.updateAlert("alert-danger", error.message, true);
                    });
            }
        },
